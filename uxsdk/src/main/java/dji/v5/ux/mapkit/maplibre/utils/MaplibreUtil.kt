...
fun fromDJILatLng(latLng: DJILatLng): LatLng {
    val displayCoordinate = DJIGpsUtils.wgs2gcjInChina(latLng)
    return LatLng(displayCoordinate.latitude, displayCoordinate.longitude, displayCoordinate.altitude)
}

fun fromLatLng(latLng: LatLng?): DJILatLng {
    latLng?.let {
        return DJIGpsUtils.gcj2wgsInChina(DJILatLng(it.latitude, it.longitude, it.altitude))
    }
