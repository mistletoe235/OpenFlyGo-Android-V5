...
open class AttitudeDisplayWidget @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    private var mDirectAslMeters: Double? = null
    ...
    private fun updateAltitude() {
        mDirectAslMeters?.takeIf { it.isFinite() }?.let {
            updateAslText(it)
            return
        }
        // Never present relative altitude as ASL. Some simulator/aircraft combinations do not
        // populate KeyHeightAboveSeaLevel; in that case an unavailable value is safer than a
        // fabricated fallback derived from ALT.
        mTvAslValue?.text = "---.-"
    }

    private fun updateAslText(aslMeters: Double, alreadyConverted: Boolean = false) {
        val displayValue = if (alreadyConverted) {
            aslMeters
        } else {
            UnitUtils.getValueFromMetricByLength(
                aslMeters.toFloat(),
                if (UnitUtils.isMetricUnits()) UnitUtils.UnitType.METRIC else UnitUtils.UnitType.IMPERIAL
            ).toDouble()
        }
        val value = String.format(Locale.US, "%06.1f", displayValue)
    ...
    override fun reactToModelChanges() {
        ...
        mCompositeDisposable.add(widgetModel.heightAboveSeaLevelProcessor.toFlowable()
            .observeOn(SchedulerProvider.ui())
            .subscribe { heightInfo: HeightAboveSeaLevelMsg ->
                mDirectAslMeters = heightInfo.height?.takeIf {
                    it.isFinite() && kotlin.math.abs(it) > 0.05 && heightInfo.quality != AltitudeDataQuality.UNKNOWN
                }
                updateAltitude()
            })
    }
