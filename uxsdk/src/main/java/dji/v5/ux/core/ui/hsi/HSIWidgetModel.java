...
public class HSIWidgetModel extends WidgetModel implements LocationListener {
    private static final long TELEMETRY_POLL_INTERVAL_MS = 200L;
    ...
    private final Runnable telemetryPoller = new Runnable() {
        @Override
        public void run() {
            LocationCoordinate3D aircraftLocation = KeyManager.getInstance().getValue(
                    KeyTools.createKey(FlightControllerKey.KeyAircraftLocation3D));
            if (aircraftLocation != null) aircraftLocationDataProcessor.onNext(aircraftLocation);
            LocationCoordinate2D homeLocation = KeyManager.getInstance().getValue(
                    KeyTools.createKey(FlightControllerKey.KeyHomeLocation));
            if (homeLocation != null) homeLocationDataProcessor.onNext(homeLocation);
            FCFlightMode flightMode = KeyManager.getInstance().getValue(
                    KeyTools.createKey(FlightControllerKey.KeyFCFlightMode));
            if (flightMode != null) flightModeProcessor.onNext(flightMode);
            Boolean multipleMode = KeyManager.getInstance().getValue(
                    KeyTools.createKey(FlightControllerKey.KeyMultipleFlightModeEnabled));
            if (multipleMode != null) multipleFlightModeEnabledProcessor.onNext(multipleMode);
            Velocity3D velocity = KeyManager.getInstance().getValue(
                    KeyTools.createKey(FlightControllerKey.KeyAircraftVelocity));
            if (velocity != null) velocityProcessor.onNext(velocity);
            Attitude attitude = KeyManager.getInstance().getValue(
                    KeyTools.createKey(FlightControllerKey.KeyAircraftAttitude));
            if (attitude != null) aircraftAttitudeProcessor.onNext(attitude);
            ProductType productType = KeyManager.getInstance().getValue(
                    KeyTools.createKey(ProductKey.KeyProductType));
            if (productType != null) productTypeDataProcessor.onNext(productType);
